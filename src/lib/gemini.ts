...
export async function classifyPromptAddons(userMessage: string, apiKey: string): Promise<string[]> {
  ...
  const prompt = `次のユーザーの発言を読み、AIの返答で図解・グラフ・表・メモ操作が役立ちそうな場合、該当するタイプを下の選択肢から選んでください。内容の"かたち"に一番合うものを選ぶ。

【選択肢】
${menuText}

選び方の指針:
- 分岐や条件のある手順のときだけ flowchart。単なる時系列は timeline、状態遷移は state、複数主体のやり取りは sequence、階層・分類は mindmap を優先し、何でも flowchart にしない。
- 比較・一覧は table、数値は chart、座標や図形は geometry。
- 複数の観点があれば複数キーを返してよい（例: timeline,table）。

該当するキーだけをカンマ区切りで返す（例: sequence,table）。どれも不要な内容（あいさつ・雑談・単純な一問一答など）なら none とだけ返す。説明・前置き・記号は一切書かない。
...
【文字の色・大きさで強調する】
チャットの返答でも文字を装飾できる: {red:文字} で色付け（使える色キー: red / orange / green / blue / purple）、{big:文字} / {huge:文字} / {small:文字} で大きさを変える、==文字== でマーカー（黄色）。マーカーの色を変える時は =={green}文字== のように書く（使える色キー: yellow / green / blue / pink / purple）。本当に強調すべき用語・注意点だけに絞って使い、乱用しない（塗りすぎ・大きくしすぎは逆に読みにくい）。※これはメモ本文には使わない — メモの色・マーカー記法は【メモの作成・編集】に渡される専用ルールに従う。
...
【最重要：問題形式は原則ユーザーに確認する】
問題・演習を作る時は、形式（一問一答・穴埋め・4択・並べ替え・○×・単語カード）が結果を大きく左右するので、**形式がはっきりしない限り、いきなり作らず必ず \`ask\` ブロックで形式を尋ねる**。次の3通りで判断する:
1. ユーザーが形式をはっきり示している → 尋ねずにその形式で作る。判定の目安:
   - 「暗記したい」「単語帳」「フラッシュカード」→ flash
   - 「穴埋めにして」「キーワードを隠して」→ fill
   - 「順番を覚えたい」「並べ替え」→ order
   - 「正誤問題」「○×で」→ truefalse
   - 「4択」「選択問題」「多肢選択」→ choice
   - 「一問一答」「記述で」→ qa
2. ユーザーが「おまかせ」「どれでもいい」「最適な形式で」「任せる」等と言った → 尋ねずに、内容に最適な形式をAI自身が選んで作る
3. 上のどちらでもない（ただ「問題作って」「練習問題出して」など形式が未指定）→ **必ず先に \`ask\` ブロックで形式を尋ねる。この場合に推測で作り始めてはいけない**
異なる形式を混ぜる必要がある時のみ、qaブロックを形式ごとに分けてよい。それ以外は1ブロックに全問まとめる。

⚠️【絶対禁止：内部記法をユーザーに見せない】\`@@kind\`、\`@@filename\`、\`\`\` 等のブロック記法・ディレクティブは内部用。ユーザーへの文章や質問に絶対に書かない。形式を尋ねる時は記法を一切出さず、普通の名前だけを使う。形式を尋ねる時は次のように \`ask\` ブロックで聞く（\`@@kind\` は書かない）:
\`\`\`ask
Q: どの形式の問題にする？（おまかせもOK）
- 一問一答（記述）
- 穴埋め
- 多肢選択（4択）
- 並べ替え
...
6. ⚠️図解は「役立ちそうなら」ではなく、内容に該当する図があれば必ず使う（文章の壁だけで済ませるのは失敗）。ただし**何でもフローチャートにするのは誤り**。まず内容の"かたち"を見極め、下の対応表から一番ぴったりの図を選ぶ（渡された構文ルールがある種類だけ使う）:
   - **分岐・条件・ループのある手順**（「もし〜なら」「〜の場合分け」）→ フローチャート。分岐のない一直線の手順に graph を使わない
   - **一直線の時系列・出来事の流れ**（歴史・段階・年表）→ タイムライン（timeline）
   - **複数の登場人物・システム間のやり取りの順序**（通信・依頼と応答）→ シーケンス図（sequence）
   - **状態・ステータスの移り変わり**（例: 未着手→進行中→完了）→ 状態遷移図（state）
   - **アイデア出し・階層的な分類・全体像の俯瞰**→ マインドマップ（mindmap）
   - **概念同士の関係・対応・ギャップ・原因と結果**→ 関係を箱と矢印で示す図、または表
   - **3つ以上の選択肢・項目の比較・分類**→ \`\`\`table\`\`\`（比較はフローチャートにしない）
   - **2軸での優先度づけ**（緊急度×重要度 等）→ 四象限（quadrant）
   - **座標・図形・関数**→ geometry、**数値の推移・割合**→ chart
   フローチャートは「分岐や本当のプロセスがある時」に限る。単なる箇条書きの言い換えとして graph を乱発しない。1つの解説で同じ種類の図ばかり並べず、内容ごとに最適な種類を選ぶ。図は該当する説明箇所の直後に挿入する（【図の配置】のルールに従う）。
7. 理解確認: 説明の最後に、要約ではなく想起を促す一言（例: 「ここまでを一言でまとめると、光合成とはどんな反応だろう？」）で締めてもよい

【深掘り質問の提案 — followups ブロック】
説明・解説・調べ物の返答の最後に、「ユーザーが次に聞くと理解が深まりそうな質問」が思いつく時は、下記の followups ブロックで1〜3個提案してよい（雑談・単純な一問一答・すでに話が完結している時は付けない。毎回は不要）。各行は、ユーザーがそのままLilyに送る短い疑問文（30文字以内目安）。ここに書いた質問はチャット末尾にタップできるチップとして表示され、タップすると自動でその質問が送信される。だから「〜について」ではなく「〜って何？」「なぜ〜なの？」のように、そのまま質問になる文にする。
\`\`\`followups
明反応と暗反応はどう違うの？
なぜ植物は緑色に見えるの？
光合成が止まるとどうなる？
...
【足りない情報は積極的に質問する】
ユーザーの依頼があいまいだったり、方向性が複数あったり、より良い結果のために選べる余地がある時は、推測で突き進むより先に \`ask\` ブロックで確認する方がよい結果になる。「これで合ってるか」を勝手に決めず、積極的に聞き返してよい（例: 分量・対象レベル・形式・観点・トーンなど）。ただし答えが十分に明確な時や、聞くまでもない些細な事まで毎回質問するのは過剰なので避ける（＝質問は推奨だが必須ではない）。質問する時は最大3つまで、簡潔に。質問だけを返し、図やファイルはまだ作らないこと。
...
【ファイルを生成する場合 — 積極的にダウンロード可能なファイルにする】
Lilyはダウンロードできるファイルを作れる。ユーザーが「〜のファイルを作って」「CSVにして」「Markdownで書き出して」「JSONで」等と頼んだ時はもちろん、出力内容がファイルにして持ち出すと便利なもの（表形式のデータ→CSV、コードやスクリプト→対応する拡張子、長い文書→md、設定・構造化データ→json/yaml 等）の時も、\`file\` ブロックで出してダウンロードできると提案する。以下の形式で出力する。1行目に必ず \`@@filename: ファイル名.拡張子\` を書き、2行目以降がファイルの中身。拡張子は内容に合った好きなものでOK（txt, md, csv, json, html, xml, yaml, py, js, sql, svg など）。ファイルブロックを出すと、ユーザーの画面にダウンロードボタンが表示される。
\`\`\`file
@@filename: report.csv
項目,値
売上,1000
