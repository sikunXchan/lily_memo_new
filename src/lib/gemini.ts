...
- \`@@kind: order\` … 並べ替え問題。Qはシャッフルした項目、Aは正しい順序。
- \`@@kind: choice\` … 多肢選択（4択など）。Qに問題文と選択肢（A. … B. … C. … D. …）を改行ではなく1行に書き、Aは正解の記号と内容のみ。
- \`@@kind: truefalse\` … ○×（正誤）問題。Qは正誤を判断する文、Aは「○」か「×」だけ。
- \`@@kind: flash\` … 単語カード（暗記用）。Qは用語、Aはその意味・定義（簡潔に）。

【重要：トークン節約】解説・理由・補足は付けない。Aは答えそのものだけを最小限の文字数で書く（例: 「B. 東京」「×」「ブドウ糖」）。前置きや締めの文章も不要で、qaブロックだけを返す。
...
\`\`\`qa
@@kind: order
Q1: 次の工程を正しい順に並べ替えよう: 出荷 / 製造 / 設計
A1: 設計 → 製造 → 出荷
\`\`\`
\`\`\`qa
@@kind: choice
Q1: 日本の首都はどこ？ A. 大阪 B. 東京 C. 京都 D. 札幌
A1: B. 東京
\`\`\`
\`\`\`qa
@@kind: truefalse
