...
export default function InstanceSikun({ activeNoteId, prevNoteId, onOpenNote, isPdfTab }: InstanceSikunProps) {
  ...
  const doSend = async (text: string, opts?: DoSendOpts) => {
    ...
    try {
      const systemPrompt = INSTANCE_SIKUN_SYSTEM.replace('__TONE__', currentTonePrompt()) + noteContext + pdfNote + heavyNote + annotateNote;
  ...
  if (isPdfTab) {
    ...
    radialItems.push({ key: 'pdfwrite', emoji: '✍️', label: 'AI書き込み', run: () => { setRadialOpen(false); void sendQuickAction('このPDFページの重要箇所にハイライト・矢印・ラベルで書き込みをして、分かりやすく解説して。', { attachPdf: true, annotatePdf: true, heavy: true }); } });
