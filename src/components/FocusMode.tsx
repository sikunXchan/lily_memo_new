...
export default function FocusMode({ onClose }: FocusModeProps) {
  ...
  // Keep the screen awake while focus mode is open (Screen Wake Lock API).
  // The lock is auto-released when the tab is hidden, so we re-acquire it
  // whenever the page becomes visible again.
  useEffect(() => {
    const nav = navigator as Navigator & { wakeLock?: { request: (t: 'screen') => Promise<WakeLockSentinel> } };
    if (!nav.wakeLock) return; // unsupported (e.g. older iOS) — silently skip
    ...
    const acquire = async () => {
      ...
      } catch { /* request can reject if not visible/allowed — ignore */ }
    };
